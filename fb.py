...
""" 
Make sure our libraries are set up properly 
"""
...
def do_interactive(fb):
    ...
    # Python 2.6 should handle code.interact without issue, so no changes are needed
    code.interact(local=gvars, banner="")

def main(fb):
    # fb.printbanner()  # Uncomment if needed for debugging
    ...
    while 1:
        try:
            ...
            do_interactive(fb)
        except Exception as e:  # Make sure we catch general exceptions
            fb.io.print_error("Error: %s" % str(e))
            break
...
def load_plugins(fb):
    ...
    # addplugins(fb, "EDE-Exploit", EDE_DIR, EDEPlugin)  # Uncomment if needed
    # addplugins(fb, "Trigger", TRIGGER_DIR, EDFPlugin)  # Uncomment if needed
